...
class NoteheadMarkup:
  ...
  def __call__(self,figures,nBeams,dots,octave,accidental,tremolo,word,line):
    ...
    if self.barPos > self.barLength:
        msg = "(notesHad=%s) barcheck fail: note crosses barline at \"%s\" with %d beams (%d skipped from %d to %d, bypassing %d), scoreNo=%d barNo=%d (but the error could be earlier)" % (' '.join(self.notesHad),figures,nBeams,toAdd,self.barPos-toAdd,self.barPos,self.barLength,scoreNo,self.barNo)
        if os.environ.get("j2ly_sloppy_bars",""):
            sys.stderr.write("Warning: "+msg+"\n")
            self.barPos = self.barPos % self.barLength
            self.barNo += 1
        else:
            errExit(msg)
